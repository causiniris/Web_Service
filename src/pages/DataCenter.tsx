// src/pages/DataCenter.tsx
import { useState, useEffect } from 'react';
import { useSearchParams } from 'react-router-dom';
import { Row, Col, Card, Select, Tag, Space, Button, Typography, Table } from 'antd';
import {
  LineChartOutlined, BarChartOutlined, PieChartOutlined, DownloadOutlined, ReloadOutlined,
  ThunderboltOutlined, HeartOutlined, FireOutlined, ExperimentOutlined,
...
export default function DataCenter() {
  ...
  useEffect(() => {
    const stopPolling = startTelemetryPolling(
      (points) => setRealData(points.slice(-60)),
      (err) => console.error('详情页数据同步异常:', err),
      8000
    );
    return () => stopPolling();
  }, [selectedDevice]);
  ...
  const formatData = (field: keyof TelemetryPoint) => {
    if (realData.length === 0) return generateTimeSeriesData(selectedDevice, field as any, 60);
  ...
  const singleData = formatData(selectedMetric);
  ...
  const allMetricData = metrics.map(m => {
    const data = formatData(m.key);
    const validData = data.length > 0 ? data : [{value: 0}];
    return { 
      ...m, data, 
      avg: +(validData.reduce((s, d) => s + d.value, 0) / validData.length).toFixed(2), 
      max: +Math.max(...validData.map(d => d.value)).toFixed(2), 
      min: +Math.min(...validData.map(d => d.value)).toFixed(2) 
    };
  });
  ...
  const multiCompareOption = {
    ...
    yAxis: allMetricData.map((m, i) => ({ type: 'value', name: m.unit, position: i > 3 ? 'right' : 'left', axisLine: { lineStyle: { color: '#334155' } }, splitLine: { show: i === 0, lineStyle: { color: '#1e293b' } }, axisLabel: { color: '#64748b' } })),
    series: allMetricData.map((m, i) => ({ name: m.label, type: 'line', yAxisIndex: i > 3 ? 1 : 0, data: m.data.map(d => d.value), smooth: true, lineStyle: { width: 2, color: m.color }, itemStyle: { color: m.color }, showSymbol: false })),
  };

  const scatterData = formatData('meniscus').map((d, i) => {
    const attData = formatData('att');
    return [d.value, attData[i]?.value || 0];
  });
  ...
  const pieOption = {
    backgroundColor: 'transparent',
    tooltip: { backgroundColor: '#1e293b', borderColor: '#334155', textStyle: { color: '#e2e8f0' } },
    series: [{ type: 'pie', radius: ['40%', '70%'], center: ['50%', '50%'], label: { color: '#94a3b8', fontSize: 11 }, data: allMetricData.map(m => ({ name: m.label, value: m.avg, itemStyle: { color: m.color } })), emphasis: { itemStyle: { shadowBlur: 10, shadowColor: 'rgba(0, 0, 0, 0.5)' } } }],
  };

  const heatmapData: number[][] = [];
  for (let i = 0; i < 7; i++) { for (let j = 0; j < 24; j++) heatmapData.push([j, i, +(Math.random() * 5 + 12).toFixed(1)]); }
  const heatmapOption = {
    backgroundColor: 'transparent',
    tooltip: { backgroundColor: '#1e293b', borderColor: '#334155', textStyle: { color: '#e2e8f0' } },
  ...
  const statsColumns = [
    ...
    { title: '最新均值', dataIndex: 'avg', key: 'avg', render: (v: number, r: typeof allMetricData[0]) => <Text style={{ color: r.color, fontWeight: 600 }}>{v}</Text> },
    { title: '近期极高', dataIndex: 'max', key: 'max', render: (v: number) => <Tag color="red">{v}</Tag> },
    { title: '近期极低', dataIndex: 'min', key: 'min', render: (v: number) => <Tag color="green">{v}</Tag> },
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h2>数据图表中心</h2>
          <p>7维核心生理/物理数据可视化分析，基于 Render 实时同步</p>
        </div>
        <Space>
          <Select value={selectedDevice} onChange={setSelectedDevice} style={{ width: 180 }} options={deviceList.map(d => ({ label: `${d.id} (${d.user})`, value: d.id }))} />
      ...
      <Row gutter={[12, 12]} style={{ marginBottom: 20 }}>
        {metrics.map(m => (
          <Col span={24 / 7} key={m.key}>
            <div className="stat-card" style={{ cursor: 'pointer', borderColor: selectedMetric === m.key ? m.color : undefined, padding: 14, textAlign: 'center' }} onClick={() => setSelectedMetric(m.key)}>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
        <Col span={24}>
          <div className="chart-card">
            <div className="chart-title"><LineChartOutlined style={{ marginRight: 8, color: currentMetric.color }} />{currentMetric.label} 趋势分析 ({currentMetric.unit}) <Tag style={{ marginLeft: 12 }} color="blue">{selectedDevice}</Tag></div>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
        <Col span={14}>
          <div className="chart-card">
            <div className="chart-title"><LineChartOutlined style={{ marginRight: 8, color: '#1890ff' }} />全指标对比趋势</div>
            <ReactECharts option={multiCompareOption} style={{ height: 350 }} />
          </div>
        </Col>
        <Col span={10}>
          <div className="chart-card">
            <div className="chart-title"><PieChartOutlined style={{ marginRight: 8, color: '#f59e0b' }} />指标均值占比</div>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
        <Col span={12}>
          <div className="chart-card"><div className="chart-title"><BarChartOutlined style={{ marginRight: 8, color: '#8b5cf6' }} />乳酸浓度周热力图</div><ReactECharts option={heatmapOption} style={{ height: 280 }} /></div>
        </Col>
        <Col span={12}>
          <div className="chart-card"><div className="chart-title"><ExperimentOutlined style={{ marginRight: 8, color: '#06b6d4' }} />半月板应力 vs 前向滑移量 散点图</div><ReactECharts option={scatterOption} style={{ height: 280 }} /></div>
        </Col>
      </Row>

      <Card title="真实数据统计汇总" style={{ borderRadius: 12 }}>
